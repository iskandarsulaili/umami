...
export function getContentSecurityPolicy() {
  ...
  // Fork customization: allow extra script sources (e.g. Cloudflare insights
  // beacon) via CSP_SCRIPT_SRC without weakening the default policy.
  const cspScriptSrc = process.env.CSP_SCRIPT_SRC || '';
  const scriptSrc = ["'self'", "'unsafe-eval'", "'unsafe-inline'", cspScriptSrc].filter(Boolean).join(' ');
